...
describe("PoolNode", () => {
  describe("findNearestInitializedTickIndex", () => {
    it("no ticks", () => {
      ...
      expect(pool.findNearestInitializedTickIndex(0)).toBe(null);
    });
    ...
    it("many ticks", () => {
      const pool = new PlainPool({
        ...
        sortedTicks: [
          ...
          { tick: -5, liquidity_delta: 0n },
          { tick: -4, liquidity_delta: 0n },
          { tick: 18, liquidity_delta: 0n },
          { tick: 23, liquidity_delta: 0n },
          { tick: 50, liquidity_delta: 0n },
        ],
        liquidity: 0n,
        sqrtRatio: 1n << 128n,
      });

      expect(pool.findNearestInitializedTickIndex(-101)).toBe(null);
