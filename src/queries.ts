...
WITH campaign_info AS (SELECT crp.campaign_id,
                       ...
                       WHERE ${chainId ? this.sql`c.chain_id = ${chainId}` : this.sql`TRUE`}
                       GROUP BY c.chain_id, crp.campaign_id, c.start_time),
...
SELECT c.slug,
       ...
       (
        ...
        (SELECT JSONB_AGG(ae::text) FROM UNNEST(c.allowed_extensions) as t(ae)) AS allowed_extensions,
