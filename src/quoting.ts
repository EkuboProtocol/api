...
function toSqrtRatio(tick: number): bigint {
...
export class PlainPool implements Node<{ initializedTicksCrossed: number }> {
  ...
  public quote({
    ...
    while (specifiedAmount > 0n && sqrtRatio !== sqrtRatioLimit) {
      const nextInitializedTick: Tick | undefined = isIncreasing
      ...
      const nextSqrtRatio = isLimited ? sqrtRatioLimit : nextTickSqrtRatio;

      // todo: compute next price from amount, liquidity
      sqrtRatio = nextSqrtRatio;

      if (sqrtRatio === nextTickSqrtRatio && nextInitializedTick) {
